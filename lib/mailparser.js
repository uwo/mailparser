...
function MailParser(options) {
    ...
    this._remainder     = ""; //REPLACE with unshift?
    this.mimeTree       = this._createMimeNode();
    this._currentNode   = this.mimeTree;
    this._currentNode.priority = "normal"; // default values for the root node
    this._fileNames     = {};
    this._multipartTree = [];
    this.mailData       = {};
    this._lineCounter   = 0;
...
function MailParser_deprecated(options){
...
utillib.inherits(MailParser, Transform);
 ...
 */
MailParser.prototype.end_deprecated = function(chunk, encoding){
...
MailParser.prototype._transform = function(chunk, encoding, callback){
  ...
  if(chunk && chunk.length){
    ...
    if(typeof setImmediate == "function"){
      setImmediate(this._process.bind(this, callback));
    }else{
      process.nextTick(this._process.bind(this, callback));
    }
  }
};

MailParser.prototype._flush = function (callback) {

  if(typeof setImmediate == "function"){
    setImmediate(flush.bind(this, callback));
  }else{
    process.nextTick(flush.bind(this, callback));
  }
  ...
  function flush(callback) {
    ...
    if(typeof setImmediate == "function"){
      setImmediate(this._processMimeTree.bind(this, callback));
    }else{
      process.nextTick(this._processMimeTree.bind(this, callback));
    }
  }
};
 ...
 */
MailParser.prototype._process = function (callback) {
...
MailParser.prototype._handleAttachmentLine = function(line){
    ...
    if(this._currentNode.stream){
        if(!this._currentNode.streamStarted){
            ...
            writable = this._currentNode.stream.write(new Buffer(line, "binary"));
        }else{
            writable = this._currentNode.stream.write(new Buffer("\r\n"+line, "binary"));
        }
...
MailParser.prototype._finalizeContents = function(){
    ...
    if(this._currentNode.stream){
        this._currentNode.stream.end();
        this._currentNode.meta.checksum = this._currentNode.stream.checksum;
        this._currentNode.meta.length = this._currentNode.stream.length;
    }
};
